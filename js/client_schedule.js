...
const EL_LINK_CONTAINER = document.querySelector(".menu-button-group");
...
function displayScheduler(show, clearInput) {
    if (show) { // Quando é selecionado um barbeiro
        EL_BLUR.classList.remove("d-none");
        EL_WINDOW_APPOINTMENT.classList.remove("d-none");
        EL_WINDOW_SPACER.classList.remove("d-none");
        EL_CLOSE_SCHEDULE_WINDOW.classList.remove("d-none");
        EL_LINK_CONTAINER.classList.add("d-none");
        EL_FOOTER.classList.add("d-none");
        EL_FOOTER_SPACER.classList.add("d-none");
    } else { // Quando sair da tela de agendamentos
        EL_BLUR.classList.add("d-none")
        EL_WINDOW_APPOINTMENT.classList.add("d-none")
        EL_WINDOW_SPACER.classList.add("d-none");
        EL_CLOSE_SCHEDULE_WINDOW.classList.add("d-none");
        EL_LINK_CONTAINER.classList.remove("d-none");
        EL_FOOTER.classList.remove("d-none");
        EL_FOOTER_SPACER.classList.remove("d-none");
        EL_BARBERS.forEach(el => { el.children.item(1).classList.remove("text-glow"); });
    }
...
function setActiveWindow(index) {
    if (checkWindowRequirement(index)) {
        ...
        addRemoveElementClass(windowPages, true, "d-none")
        addRemoveElementClass(warningMsg, true, "d-none");
...
function checkWindowRequirement(index) {
    ...
    if (index == 1) { // conferir calendario
        if ((diaAgendado <= 0 || diaAgendado >= 32)) {
            warningMsg[index - 1].classList.remove("d-none");
        } else return true
    }
...
function createScheduleTime(time, minute) {
    ...
    sch_description.classList.add("d-none");
...
function disableSchedule(timeToDisable, minuteToDisable, ammount, isOccupied) {
    ...
    for (let i = 0; i < ammount; i++) {
        ...
        if (indexToChance < allTimeInSchedule.length) {
            ...
            allTimeInSchedule[indexToChance].firstChild.lastChild.classList.remove("d-none"); // msg de indisponivel
            ...
            if (entries.length > 0) {
                if (i == 0) { entries[i].classList.add(rowType + "-top"); } else
                    if (i == ammount - 1) { entries[i].classList.add(rowType + "-base"); } else { entries[i].classList.add(rowType); }
            }
        } else {
            return;
        }
